...
\section{Simulations and measurements: The graphical user interface}\index{GUI}\index{GUI}
To be completed (Dinesh).

\section{Simulations and measurements: Manual setup}\index{Measurements}\index{Files}\index{Application}\label{sec:Simulation_Measurements}
...
\begin{figure}[!h]
\centering\includegraphics[width=0.98\textwidth]{input_files.pdf}
...
\begin{enumerate}
	...
	\item \TimeAgent
	\item \DomainAgent
	\item \NegotiatorAgent \; (in interdependent case)
\end{enumerate}
Hence a SFINA experiment can be set up in a modular way, allowing for simple functionality extensions.
...
\begin{figure}[!h]
\centering\includegraphics[width=0.6\textwidth]{active_state.pdf}
...
\subsubsection{Iterations and output}\index{Iterations}\index{Output}
...
Has to implement the following interface in order to function with the SFINA framework:
...
The following default implementation exist:
	...
	\end{enumerate}
%------------------------------------------------
\section{\CommunicationAgent}\index{Communication Agent! \CommunicationAgent}\label{sec:communication_agent}	
Abstract class which extends SimpleTimeSteppingAgent. Thus it has the same core responsibility as \TimeAgent.
...
\begin{enumerate}
	\item InterdependentCommunicationAgent: Allows for multiple networks to be connected, thus manages their synchronization and exchange of information. The execution logic is of paralel nature: Each iteration of all networks happens in parallel, after which the network update each other. 
...
Responsible to resolve event conflicts in interdependent network simulations.
...
For obtaining a deeper understanding of SFINA it is necessary to become acquainted with some SFINA related terminology.
...
During each time step several simulation iterations can be performed, depending on the logic implemented in the \TimeAgent . I.e. one can decided to perform another simulation, if the simulation did not converge in the previous iteration.
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\begin{enumerate}[label=(\alph*)]
			...
			\item or to progress \MainAgent to the next time step through calling \textit{progressToNextTimeStep()}.
		\end{enumerate}  
	\end{itemize}
\end{enumerate}
...
\section{Implementing a new Application Agent}\index{Agent}
...
\section{Approaches for a new \Backend{}}\index{\Backend{}}\index{\Backend{} !Interpss}\index{\Backend{} !Matpower}\index{\Backend{} !Interface}\label{sec:backend}
...
\section{Overview of necessary adjustments}
...
\section{FlowNetworkDataTypesInterface}
