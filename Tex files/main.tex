...
\section{The graphical user interface}\index{GUI}\index{GUI}
...
\begin{enumerate}
	\item A \MainAgent, which is extended to different functionality, for example BenchmarkAgent for measurements or the PowerCascadeAgent.
	...
	\item A \DomainAgent, for example InterpssBackend or MatpowerBackend (requiring a Matlab installation) in the case of a power network simulation.
\end{enumerate}
...
The callBackend() method executes the backend, which is automatically selected according to the FlowDomainAgent chosen in the experiment class. Currently implemented backends are shown in the followig table.
\begin{table}[h]
	\centering
	\begin{tabular}{|l| l |l|}
	\hline
