...
\subsection{The graphical user interface}\index{GUI}\index{GUI}
Each SimulatedExperiment e.g. PowerFlowAgent.java needs to implement the Runnable Interface and the method run(). The GUI class contains the SimulatedExperiment class. To create new GUI for a new experiment, users can simply instantiate GUI class by passing the SimulatedExperiment class as a parameter to constructor of GUI. 

\paragraph{Network Visualization} SFINANetworkGenerator is implemented as a separate library and incorporated into the GUI to create new networks of edit existing networks. So, to use the GUI, users need to include compiled SFINANetworkGenerator and obviously other usual libraries of SFINA Framework. SFINANetworkGenerator makes use of Jung Graph Library and its examples to create or edit existing networks.

\paragraph{Create new network} To create new network from scratch, users need to specify node and link fields in the beginning as shown in images createNetworkFromScratch.png and specifyNodeAndLinkFieldsForNetworkCreationFromScratch.png. To do this, users need to click on Menu->Edit->Create Network.
Existing networks can be loaded and edited within the GUI. Menu->Edit->Edit Network
 
\paragraph{Running an experiment with the GUI} Users can modify the configurations for current experiment class by clicking in Menu->Experiment->Configurations. The configurations will be saved to file and used while running the experiment.
To run a specific experiment, users can either click on Menu->Experiment->Run or  Right Click on the experiment directory seen at the folder tree on side panel and click on Run Experiment pop up menu item. Users can double click on text files on folder tree on side panel to view the information in experiment files. The log of experiment is displayed in “Debug” panel.
